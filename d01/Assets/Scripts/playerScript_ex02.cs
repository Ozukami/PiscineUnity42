...
public class playerScript_ex02 : MonoBehaviour {
	...
	void OnTriggerEnter2D ( Collider2D collider ) {
		...
		if (collider.tag == "Teleport") {
			Debug.Log("* Teleporting to an unknown location *");
			transform.position = collider.transform.GetChild(0).transform.position;
		}
	}
	...
	void OnCollisionEnter2D ( Collision2D collider ) {
		...
		if (collider.transform.tag == "Switch") {
			switchScript ss = collider.gameObject.GetComponent<switchScript>();
			if (this.tag == ss.targetColor || ss.targetColor == "All") {
				ss.open();
			} else if (ss.targetColor == "PlayerColor") {
				ss.openColor(this.tag);
			} else if (ss.targetColor == "None") {
				ss.changeColors();
			}
		}
	}
