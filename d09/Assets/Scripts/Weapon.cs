...
public class Weapon : MonoBehaviour {
	...
	public void Fire () {
		if (time >= FireRate) {
			...
			RaycastHit hit;
			if (Physics.Raycast(Camera.main.transform.position, Camera.main.transform.forward, out hit, 100, ~(1 << 8))) {
				...
				if (hit.collider.gameObject.layer == 9) {
					if (hit.collider.gameObject.GetComponent<Enemy>() != null) {
						...
						hit.collider.gameObject.GetComponent<Enemy>().GoAtPos(transform.position);
					}
				}
			}
