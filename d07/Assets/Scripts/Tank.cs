...
public class Tank : MonoBehaviour {
	...
	void OnTriggerStay (Collider other) {
		if (other.gameObject.tag == "Tank" && target == null) {
			...
			StartCoroutine(coroutine);
				...
				&& Vector3.Distance(transform.position, other.transform.position) < Vector3.Distance(transform.position, target.transform.position)) {
			target = other.gameObject;
		}
	}
    ...
    void Fire () {
        ...
        if (type == 0) { gunShot.Play(); }
		else { noHit.Play(); }
        RaycastHit hit;
		Vector3 randomVector = new Vector3(Random.Range(-4, 4), Random.Range(-4, 4), Random.Range(-4, 4));
		Vector3 fireDirection = target.transform.Find("canon").transform.position - canon.transform.position + randomVector;
        if (Physics.Raycast(canon.transform.position, fireDirection, out hit)) {
            if (hit.collider.gameObject.layer == 8) {
                ...
                if (hit.collider.GetComponent<Player>().IsDead()) {
					target = null;
					isPatrolling = true;
					StopAllCoroutines();
					GoToNextPoint();
                ...
                Debug.DrawRay(canon.transform.position, fireDirection, Color.green);
                hit.collider.transform.root.gameObject.GetComponent<Tank>().TakeDamage(5 + (type * 10));
            } else {
